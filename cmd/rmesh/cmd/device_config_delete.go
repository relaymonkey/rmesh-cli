...
Personal rows (` + "`cloud:<n>/mine/<label>`" + ` or bare ` + "`cloud:<n>/<label>`" + `)
require you to be the owner. Network templates (` + "`cloud:<n>/template/<label>`" + `)
require an elevated network role (admin, operator, or deployer).
...
Examples:
  ...
  rmesh device config delete --from cloud:home/mine/eu-868
...
func runDeviceConfigDelete(cmd *cobra.Command, _ []string) error {
	...
	if detail, err := c.GetDeviceConfig(ctx, netID, cfgID, false); err == nil {
		switch detail.OwnerKind {
		case "network":
...
func init() {
	...
	f.StringVar(&deleteFlags.from, "from", "", "Cloud source: cloud:<network>/<label> (or cloud:<network>/mine|template/<label>)")
